    ...
    public void onUpdate(UpdateEvent event) {
        if (universalStarted) {
            if (mc.thePlayer.onGround || mc.thePlayer.fallDistance > 8f) {
                ...
                if (isOverVoid()) {
                    universalFlag = true;
                    PacketUtil.sendNoEvent(new C03PacketPlayer.C04PacketPlayerPosition(groundX, groundY + 0.1, groundZ, false));
                }
            }
        } else if (mc.thePlayer.offGroundTicks == 1 && !mc.thePlayer.onGround && mc.thePlayer.motionY < 0) {
            if (isOverVoid()) {
