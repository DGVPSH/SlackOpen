...
public class SilentHitbox extends Module {
    ...
    public SilentHitbox() {
        addSettings(range, fov, fovRender, expand, onMouse, onlyCombat);
    }
    ...
    public void onUpdate (UpdateEvent event) {
        ...
        if ((onMouse.getValue() && !mc.gameSettings.keyBindAttack.isKeyDown()) || (onlyCombat.getValue() && !AttackUtil.inCombat)) {
            if (enabled) {
                disable();
            }
            return;
